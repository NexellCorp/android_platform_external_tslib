...
int getMaxMinValues(int ts_fd)
{
    ...
    if(ioctl(ts_fd, EVIOCGABS(ABS_X), &info_X)) {
        ALOGE("Error reading absolute controller %d for fd %d\n",
             ABS_X, ts_fd);
        return -1;
    }
...
// Get determinant of matrix -- check if determinant is too small
    ...
    if(det < 0.1 && det > -0.1) {
        ALOGE("ts_calibrate: determinant is too small -- %f\n",det);
        return 0;
    }
...
int calibrateAndroid(int *a, int ts_fd)
{
    ...
    int i, j;
    char cal_buffer[256];
	int elements[20] = {85, 535, 946, 529, 944, 68, 74, 72, 513, 302, 50, 50, 974, 50, 974, 550, 50, 550, 512, 300};
    ...
    if((calfile = getenv("TSLIB_CALIBFILE")) == NULL) calfile = defaultcalfile;
        ...
        if(pcal_fd < 0){
            ALOGE("open failed for pointercal file set default pointcal");
        }
		else
		{
        ...
        if(getMaxMinValues(ts_fd) < 0){
            ALOGE("Cannot access max and min values of touch screen from kernel");
            return -1;
        }
        ...
        for(i=0; i< ((sizeof(elements))/sizeof(int))/2; i+=2){
            ...
            if(0) {
                ALOGI("Value of X[%d]=%d in ts_Calibrate\n", j, cal.x[i/2]);
                ALOGI("Value of Y[%d]=%d in ts_Calibrate\n", j, cal.y[i/2]);
                ALOGI("Value of refX[%d]=%d in ts_Calibrate\n", j, cal.xfb[i/2]);
                ALOGI("Value of refY[%d]=%d in ts_Calibrate\n", j, cal.yfb[i/2]);
            }
            j++;
        }

        ALOGV("tslib: going to call perform_calibration() \n");
        if (perform_calibration (&cal, a)) {
            ALOGV("tslib:perform_calibration succeeded\n ");

            ALOGV("Calibration constants: ");
            for (i = 0; i < 7; i++) {
                ALOGV("%d ", a[i]);
            }
